...
from wiscsim.global_hotness import *
import config
import ftlbuilder
...
# PPN_BYTES = 4
SUBLPN_BYTES = 1  # = 1 if use FramePLR
PPN_BYTES = 2
FLOAT16_BYTES = 2
...
class Ftl(ftlbuilder.FtlBuilder):
    ...
    def end_ssd(self):
        ...
        # log_msg('non_consecutive_length: %d' %(non_consecutive_length))
        # log_msg('non_consecutive_point_count: %d' %(non_consecutive_point_count))
        # log_msg('consecutive_point_count: %d' %(non_consecutive_point_count))
        crb_distribution = []
        for frame in self.metadata.mapping_table.frames.values():
            crb_size = 0
    ...
    def clean(self, forced=False, merge=True):
        ...
        # for block , ratio in sorted_blocks[:100]:
        #     erased_pbns.append(block)
        #     validate_pages += self.metadata.pvb.get_valid_pages(block)
...
class BlockValidityCounter(object):
    ...
    def __init__(self, conf):
        ...
        self.total_block = len(self.free_block_list)
...
class FrameLogPLR:
    ...
    def __init__(self, conf, metadata, counter, gamma, max_size=1*1024**2, frame_length=256):
        ...
        SUBLPN_BYTES = 1
        global PPN_BYTES
        PPN_BYTES = conf['PPN_LEN']
        global LPN_BYTES
        LPN_BYTES = conf['LPN_LEN']
        self.conf = conf
    ...
    def allocate_ppn_for_frame(self, frame_no):
        if not self.current_trans_block or self.current_trans_page_offset == self.conf.n_pages_per_block:
    ...
    def flush(self):
        ...
        # log_msg("hot_lpn_count:%d" % ( hot_lpn_count ))
