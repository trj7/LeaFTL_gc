...
def parse_events(filename, page_size, recorder=True, start_lineno=0, lineno=float('inf'), max_writes = float('inf'), max_write_size = float('inf'), write_only=False, format="MSR", capacity = 2*TB, shift_range=False):
    ...
    format_config = {"MSR" : (1, 100, ","), "blktrace" : (512, 1000**3, " "), "systor" : (1, 1000**3, ","), "normal" : (1, 1000, " "), "FIU" : (512, 1, " "), "Financial" : (1, 1000**3, ",", 512),"simple": (512, 1, " ")} 
    ...
    with open(filename) as fp:
        ...
        for i, raw in enumerate(fp):
            ...
            if format == "MSR":
                ...
                if len(line) == 6:
                    t, dev, offset, size, mode, hotness = line
                    hotness = int(hotness)
                    if hotness == 1:
                        update_var(offset , offset_scale , page_size, size)
                else:
                    t, dev, offset, size, mode = line
                t, offset, size, mode = int(t), int(offset), int(size), int(mode)
            # shift timestamp
            if not t_start:
                t_start = t
            t -= t_start
